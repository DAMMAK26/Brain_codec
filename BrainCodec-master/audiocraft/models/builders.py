...
def get_quantizer(quantizer: str, cfg: omegaconf.DictConfig, dimension: int) -> qt.BaseQuantizer:
    klass = {"no_quant": qt.DummyQuantizer, "rvq": qt.ResidualVectorQuantizer}[quantizer]
...
def get_encodec_autoencoder(encoder_name: str, cfg: omegaconf.DictConfig):
    ...
    elif encoder_name in ["seanet_fMRI", "seanet_noisyfMRI"]:
        kwargs = dict_from_config(getattr(cfg, "seanet"))
        encoder_override_kwargs = kwargs.pop("encoder")
        decoder_override_kwargs = kwargs.pop("decoder")
        encoder_kwargs = {**kwargs, **encoder_override_kwargs}
        decoder_kwargs = {**kwargs, **decoder_override_kwargs}
        encoder = audiocraft.modules.SEANetfMRIEncoder(**encoder_kwargs)
        decoder = audiocraft.modules.SEANetfMRIDecoder(**decoder_kwargs)
        return encoder, decoder
    elif encoder_name == "seanet_fMRI_prvq":
        kwargs = dict_from_config(getattr(cfg, "seanet"))
        encoder_override_kwargs = kwargs.pop("encoder")
        decoder_override_kwargs = kwargs.pop("decoder")
        encoder_kwargs = {**kwargs, **encoder_override_kwargs}
        decoder_kwargs = {**kwargs, **decoder_override_kwargs}
        encoder = audiocraft.modules.SEANetfMRIPRVQEncoder(**encoder_kwargs)
        decoder = audiocraft.modules.SEANetfMRIPRVQDecoder(**decoder_kwargs)
        return encoder, decoder
    else:
...
def get_compression_model(cfg: omegaconf.DictConfig) -> CompressionModel:
    ...
    elif cfg.compression_model == "encodec_fMRI":
        kwargs = dict_from_config(getattr(cfg, "encodec"))
        encoder_name = kwargs.pop("autoencoder")
        quantizer_name = kwargs.pop("quantizer")
        encoder, decoder = get_encodec_autoencoder(encoder_name, cfg)
...
def get_downstream_fMRI_model(cfg: omegaconf.DictConfig) -> DownstreamfMRIModel:
    if cfg.downstream_fMRI_model == "linearbaseline":
        kwargs = dict_from_config(getattr(cfg, "linear"))
        return LinearBaseline(**kwargs).to(cfg.device)
    if cfg.downstream_fMRI_model == "linearbaselineforhidden":
        kwargs = dict_from_config(getattr(cfg, "linear"))
        return LinearBaselineforHidden(**kwargs).to(cfg.device)
    else:
        raise KeyError(f"Unexpected compression model {cfg.compression_model}")
...
def get_lm_model(cfg: omegaconf.DictConfig) -> LMModel:
    ...
    elif cfg.lm_model == "transformer_raw_lm":
        kwargs = dict_from_config(getattr(cfg, "transformer_raw_lm"))
        attribute_dropout = dict_from_config(getattr(cfg, "attribute_dropout"))
        cls_free_guidance = dict_from_config(getattr(cfg, "classifier_free_guidance"))
        cfg_prob, cfg_coef = (
            cls_free_guidance["training_dropout"],
    ...
    elif cfg.lm_model == "transformer_raw_lm_downstream":
        kwargs = dict_from_config(getattr(cfg, "transformer_raw_lm"))
        attribute_dropout = dict_from_config(getattr(cfg, "attribute_dropout"))
        cls_free_guidance = dict_from_config(getattr(cfg, "classifier_free_guidance"))
        cfg_prob, cfg_coef = (
    ...
    elif cfg.lm_model == "transformer_lm_fMRI":
        kwargs = dict_from_config(getattr(cfg, "transformer_lm_fMRI"))
        n_q = kwargs["n_q"]
        q_modeling = kwargs.pop("q_modeling", None)
        codebooks_pattern_cfg = getattr(cfg, "codebooks_pattern")
        attribute_dropout = dict_from_config(getattr(cfg, "attribute_dropout"))
    ...
    elif cfg.lm_model == "transformer_lm_fMRI_downstream":
        kwargs = dict_from_config(getattr(cfg, "transformer_lm_fMRI"))
        n_q = kwargs["n_q"]
        q_modeling = kwargs.pop("q_modeling", None)
        codebooks_pattern_cfg = getattr(cfg, "codebooks_pattern")
...
def get_conditioner_provider(output_dim: int, cfg: omegaconf.DictConfig) -> ConditioningProvider:
    ...
    for cond, cond_cfg in dict_cfg.items():
        ...
        if model_type == "t5":
            ...
            conditioners[str(cond)] = CLAPEmbeddingConditioner(output_dim=output_dim, device=device, **model_args)
        else:
...
def get_debug_lm_model(device="cpu"):
    ...
    fuser = ConditionFuser({"cross": ["description"], "prepend": [], "sum": [], "input_interpolate": []})
    lm = LMModel(
        pattern,
        condition_provider,
        fuser,
